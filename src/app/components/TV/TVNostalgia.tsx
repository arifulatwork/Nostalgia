  ...
  class VCREffect {
    ...
    private renderTrackingNoise(radius = 2) {
      ...
      for (let i = 0; i <= num; i++) {
        ...
        let y1 = this.getRandomInt(miny += 3, canvas.height);
        let y2 = this.getRandomInt(0, miny2 -= 3);
    ...
    private renderTail(ctx: CanvasRenderingContext2D, x: number, y: number, radius: number) {
      ...
      for (let i = 0; i < n; i++) {
        let r = this.getRandomInt(radius - 0.01, radius);
        let dx = this.getRandomInt(1, 4) * dir;
        radius -= 0.1;
